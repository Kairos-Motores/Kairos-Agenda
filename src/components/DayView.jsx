...
export const DayView = ({ selectedDate, viewType, getEventsForDay, holidays, allUsers = [], eventTypes = [], onEdit, dayViewMode = 'timeline' }) => {
                        ...
                        <div key={hour} style={{ display: 'flex', borderBottom: '1px solid var(--border-color)', minHeight: '65px', width: '100%', boxSizing: 'border-box' }}>
                            ...
                            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '4px', padding: '5px', backgroundColor: eventsInHour.length > 0 ? 'var(--bg-tertiary)' : 'transparent', minWidth: 0, boxSizing: 'border-box' }}>
                                {eventsInHour.map(event => {
                                    ...
                                    return (
                                        <div key={event.cr4a1_agenda_kairosid} onClick={(e) => onEdit(event, e.currentTarget.getBoundingClientRect())} className="day-view-event boing-effect" style={{
                                            backgroundColor: userColor, color: 'white', padding: '6px 12px', fontSize: '12px', cursor: 'pointer', overflow: 'hidden', width: '100%',
                                            ...
                                            <div style={{ fontWeight: '700', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                                                {typeLayer.layer === 'icone' && typeLayer.emoji && <span style={{ marginRight: '4px' }}>{typeLayer.emoji}</span>}
                                                {event.cr4a1_titulo}
                                            </div>
                            ...
                            <div key={targetDateStr} style={{ flex: 1, minWidth: viewType === 'day' ? '100%' : '120px', borderRight: '1px solid var(--border-color)', display: 'flex', flexDirection: 'column' }}>
                                ...
                                <div style={{ minHeight: '30px', padding: '4px', backgroundColor: 'var(--bg-primary)', display: 'flex', flexDirection: 'column', gap: '2px' }}>
                                    ...
                                    {allDayEvents.map(event => {
                                        ...
                                        const typeLayer = getEventTypeLayer(event.cr4a1_tipo, eventTypes);
                                        return (
                                            <div key={event.cr4a1_agenda_kairosid} onClick={(e) => onEdit(event, e.currentTarget.getBoundingClientRect())} style={{
                                                backgroundColor: userColor, color: 'white', padding: '2px 4px', borderRadius: '4px', fontSize: '10px', cursor: 'pointer', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis',
                                                ...(typeLayer.layer === 'borda' ? typeLayerBorderStyle(typeLayer.color) : {}),
                                                ...(typeLayer.layer === 'padrao' ? typeLayerPatternStyle(typeLayer.color) : {}),
                                            }}>
                                                {typeLayer.layer === 'icone' && typeLayer.emoji ? typeLayer.emoji + ' ' : ''}
